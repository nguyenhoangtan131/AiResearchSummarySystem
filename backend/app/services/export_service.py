...
class ExportService:
    ...
    def _render_body_paragraphs(self, pdf: FPDF, content: str) -> None:
        paragraphs = [item.strip() for item in (content or "").split("\n") if item.strip()]
        for paragraph in paragraphs:
            indented_paragraph = f"     {paragraph}"
            pdf.set_x(pdf.l_margin)
            pdf.multi_cell(pdf.epw, 7.2, indented_paragraph, align='J')
            pdf.ln(1.5)

    def generate_article_pdf(self, article_title: str):
        pdf = AcademicPDF(unit='mm', format='A4')
        ...
        pdf.set_y(42)
        pdf.multi_cell(0, 10, article_title, align='C')
        pdf.ln(14)

        for section in self.numbered_sections:
            if pdf.page_no() > 1 or pdf.get_y() > 80:
                pdf.add_page()

            chapter_number = section["number"]
            display_title = str(section["title"]).upper()
            ...
            pdf.set_y(max(pdf.get_y(), 38))
            pdf.set_font("DejaVu", "B", 16)
            pdf.cell(0, 9, f"CHƯƠNG {chapter_number}", ln=True, align='C')
            pdf.ln(4)
            pdf.set_font("DejaVu", "B", 15)
            pdf.multi_cell(0, 9, display_title, align='C')
            pdf.ln(10)
            pdf.set_font("DejaVu", "", 13)
            self._render_body_paragraphs(pdf, str(display_content))

        if self.final_source_list:
            pdf.add_page()
            pdf.set_y(30)
            pdf.set_font("DejaVu", "B", 16)
            pdf.cell(0, 10, "TÀI LIỆU THAM KHẢO", ln=True, align='C')
            pdf.ln(8)
            ...
            for i, source in enumerate(self.final_source_list, start=1):
                ...
                pdf.multi_cell(usable_w, 6.5, f"{pub}, {year}. \"{title}\".")

                if link:
                    pdf.set_x(content_x)
                    pdf.set_font("DejaVu", "", 10)
                    pdf.multi_cell(usable_w, 5, link, link=link)
